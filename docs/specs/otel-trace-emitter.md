...
2. **`record(obs)` choke point** — collapse the three finalization sites in `forward()`
   ...
   so the emitter is added at one site, not three. *(done — merged in #18, commit `95c3199`,
   as Plan A along with the observe-only `session_id` groundwork. Plan B below is this
   branch.)*
