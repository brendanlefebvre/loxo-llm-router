...
- **Golden harness fixtures** — the harness-as-spec made executable. Capture sanitized real request bodies from OpenCode and omp sessions (and, once B5 lands, Claude Code); earlier Pi fixtures are retained as informal regression fixtures with no acceptance-run obligation (main turns, title/summarize chores, compaction, tool-heavy turns), pinned per harness version. Replay them against the app with a stubbed upstream; assert routing decisions, injected fields (`cache_control`, `stream_options.include_usage`), streaming semantics, and error surfaces. When a harness updates and its request shapes drift, the fixtures say so before a user session does.
